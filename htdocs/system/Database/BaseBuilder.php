...
class BaseBuilder
{
     ...
     *
     * @var list<null|bool> true=protect, false=not protect
     */
    public $QBNoEscape = [];
     ...
     * @param null|bool                         $escape Whether to protect identifiers
     *
     * @return $this
     */
    public function select($select = '*', ?bool $escape = null)
    {
        ...
        if (is_string($select)) {
            $select = (false === $escape) ? [$select] : explode(',', $select);
        }
     ...
     *
     * @param null|array|BaseBuilder|(\Closure(BaseBuilder): BaseBuilder) $values The values searched on, or anonymous function with subquery
     *
     * @return $this
     */
     ...
     * @param null|bool           $escape Whether to escape values
     *
     * @return $this
     */
    ...
    public function countAllResults(bool $reset = true)
    {
        ...
        if (true === $this->QBDistinct || !empty($this->QBGroupBy)) {
    ...
    public function upsert($set = null, ?bool $escape = null)
    {
        ...
        if (null === $set && !is_array(current($this->QBSet))) {
            $set = [];
     ...
     *
     * @param null|array|object $set a dataset
     *
     * @return false|int|list<string> Number of affected rows or FALSE on failure, SQL array when testMode
     *
     * @throws DatabaseException
     ...
     * @param null|array                       $ignore       ignores items in set
     *
     * @return $this
     */
    public function updateFields($set, bool $addToDefault = false, ?array $ignore = null)
    {
        if (!empty($set)) {
            ...
            foreach ($set as $key => $value) {
                ...
                if (null === $ignore || !in_array($key, $ignore, true)) {
                    if ($addToDefault) {
            ...
            if (false === $addToDefault && isset($this->QBOptions['updateFieldsAdditional'], $this->QBOptions['updateFields'])) {
     ...
     * @param null|array|string  $columns an array or comma delimited string of columns
     */
    public function setQueryAsData($query, ?string $alias = null, $columns = null): BaseBuilder
    {
        ...
        if (is_string($query)) {
            if (null !== $columns && is_string($columns)) {
     ...
     *
     * @param null|array|object        $set
     * @param null|array|RawSql|string $where
     *
     * @throws DatabaseException
     */
     ...
     *
     * @param null|array|object        $set         a dataset
     * @param null|array|RawSql|string $constraints
     *
     ...
     *
     * @param null|array|object $set         a dataset
     * @param null|array|RawSql $constraints
     *
     * @return false|int|list<string> Number of rows affected or FALSE on failure, SQL array when testMode
     */
    ...
    protected function whereHaving(string $qbKey, $key, $value = null, string $type = 'AND ', ?bool $escape = null)
    {
        ...
        foreach ($keyValue as $k => $v) {
            ...
            } elseif (!$this->hasOperator($k) && 'QBHaving' !== $qbKey) {
    ...
    protected function _whereIn(?string $key = null, $values = null, bool $not = false, string $type = 'AND ', ?bool $escape = null, string $clause = 'QBWhere')
    {
        ...
        if (null === $values || (!is_array($values) && !$this->isSubquery($values))) {
    ...
    protected function _like($field, string $match = '', string $type = 'AND ', string $side = 'both', string $not = '', ?bool $escape = null, bool $insensitiveSearch = false, string $clause = 'QBWhere')
    {
        ...
        if ($field instanceof RawSql) {
            ...
            if (true === $escape && '' !== $this->db->likeEscapeStr) {
    ...
    protected function _update(string $table, array $values): string
    {
        ...
        return 'UPDATE '.$this->compileIgnore('update').$table.' SET '.implode(', ', $valStr)
            .$this->compileWhereHaving('QBWhere')
            .$this->compileOrderBy()
            .(false !== $this->QBLimit ? $this->_limit(' ', true) : '');
    }
    ...
    protected function compileSelect($selectOverride = false): string
    {
        ...
        } elseif (false !== $this->QBLimit || $this->QBOffset) {
            $sql = $this->_limit($sql."\n");
        }
    ...
    protected function compileGroupBy(): string
    {
        if (!empty($this->QBGroupBy)) {
            foreach ($this->QBGroupBy as &$groupBy) {
                ...
                $groupBy = (false === $groupBy['escape'] || $this->isLiteral($groupBy['field']))
                    ? $groupBy['field']
    ...
    protected function compileOrderBy(): string
    {
        if (is_array($this->QBOrderBy) && [] !== $this->QBOrderBy) {
            foreach ($this->QBOrderBy as &$orderBy) {
                if (false !== $orderBy['escape'] && !$this->isLiteral($orderBy['field'])) {
     ...
     */
    protected function resetRun(array $qbResetItems): void
    {
